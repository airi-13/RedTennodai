...
- `/students`: 生徒登録(氏名・学校・学年・性別・ログインID/パスワードの発行)、定期スケジュール管理、スプレッドシートからのタブ区切り貼り付けによる一括登録
- `/requests`: 生徒からの欠席・振替申請を確認し、承認/却下する(承認してもattendance_recordsは自動更新されない。手動で`/attendance`から反映する運用)
- `/admin-calendar`: 休講・特別開講の1日単位の設定、塾からのお知らせ、生徒の学校行事(学校名を入力すると自動でマスタ登録)、管理者用TODOの管理
- `/materials`: 使用テキスト・学年別料金の登録
- `/login` `/my` `/my/request`: 生徒本人がログインし、月次カレンダー(自分の授業・学校行事・塾のお知らせ)を見て、欠席/振替を申請できる。`/my/history`で週間スケジュール・出欠履歴、`/my/textbooks`で使用テキスト、`/my/pricing`で料金シミュレーションも見られる
