...
**注意**: 同じRenderワークスペースに`kakomon-app`という別プロジェクトのサービスが存在するが、本プロジェクトとは無関係。

管理画面パスワード等の環境変数は`.dev.vars.example`(必要なキーの一覧)を参照し、実際の値はCloudflareダッシュボードのVariables and Secrets、またはSupabaseダッシュボード(Settings > API Keys)で確認する。このファイルには機密情報を書かない方針。
...
- **Cloudflare**: ダッシュボードのMembers設定から招待
- **Render**(参考・従来環境): ワークスペースのSettings(People)から招待。`kakomon-app`も見える点に注意
...
- Next.js(App Router, TypeScript, Tailwind v4) / Supabase(Postgres, Auth) / **Cloudflare Workers**(OpenNext経由でホスティング。従来のRenderからも`npm run build`/`npm run start`でデプロイ可能) / GitHub
...
- `/attendance`: 日付を選ぶとその日の各コマの予定生徒が一覧表示され、クリックで出席/欠席/遅刻/振替/無断欠席と科目をその場で変更できる。振替(元コマ)・無断欠席の行はグレーアウトされるが一覧からは消えない。振替で追加されたコマにも独立して出欠登録できる(来たことを記録可能)
- `/students`: 生徒登録(氏名・学校・学年・性別・ログインID/パスワードの発行)、既存生徒の編集、定期スケジュール管理、スプレッドシートからのタブ区切り貼り付けによる一括登録
- 生徒の欠席・振替登録(`/my`カレンダー上の授業タップ→ポップアップ、または`/my/request/makeup`): **管理者の承認なしに、生徒が登録した時点でその場に確定し`attendance_records`に即時反映される**。ルール: (1)対象授業開始5分前を過ぎると登録不可 (2)振替日は元の授業日から4週間以内のみ選択可 (3)振替日が未定でも欠席/振替として登録でき、後から振替日だけ追加できる。`/requests`は履歴確認+誤登録の取り消し用(取り消すと出欠記録も削除される)
- `/admin-calendar`: 休講・特別開講の1日単位の設定、塾からのお知らせ、生徒の学校行事(学校名を入力すると自動でマスタ登録)、管理者用TODO、**生徒トップページ用のお知らせ(日付に紐づかない一般的な掲示)**の管理
- `/materials`: 使用テキスト(70件初期データ投入済み)・区分/科目/学年での絞り込み・学年別料金の登録
- `/login` `/my`: **生徒のトップページ**。管理者からのお知らせ欄+月次カレンダー(自分の授業・学校行事・塾のお知らせ)を表示。授業の色分けは 白=通常 / 赤=欠席 / グレー=振替元・無断欠席 / 青=振替先。`/my/history`で週間スケジュール・出欠履歴、`/my/textbooks`で使用テキスト、`/my/pricing`で料金シミュレーションも見られる
