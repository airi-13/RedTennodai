...
export function TextbooksView({ textbooks }: { textbooks: Textbook[] }) {
  ...
  return (
    <div className="mx-auto max-w-2xl p-4">
      ...
      {textbooks.length === 0 ? (
        ...
        <>
          <div className="mb-5 flex flex-wrap items-end gap-2 rounded-lg border p-3">
            <label className="flex flex-col gap-1 text-xs text-neutral-500">区分
            ...
            <label className="flex flex-col gap-1 text-xs text-neutral-500">教科
            ...
            <label className="flex flex-col gap-1 text-xs text-neutral-500">学年
          ...
          {filtered.length === 0 ? <p className="text-sm text-neutral-500">該当するテキストがありません。</p> : (
              ...
              {TEXTBOOK_LEVELS.map((level) => {
                ...
                return <section key={level}>
                  ...
                  {TEXTBOOK_SUBJECTS.map((subject) => {
                    ...
                    return <div key={subject} className="mb-4">
                      <h3 className="mb-2 text-sm font-semibold text-neutral-600">{getTextbookSubjectLabel(level, subject)}</h3>
                      <ul className="space-y-2">
                        {subjectBooks.map((b) => <li key={`${subject}-${b.id}`}>
                          <button type="button" onClick={() => setSelectedTextbook(b)} className="w-full rounded-lg border p-3 text-left transition hover:border-neutral-400 hover:bg-neutral-50">
      ...
      {selectedTextbook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="textbook-detail-title" onClick={() => setSelectedTextbook(null)}>
          <div className="w-full max-w-md rounded-xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <h2 id="textbook-detail-title" className="text-lg font-semibold text-neutral-800">{selectedTextbook.title}</h2>
              <button type="button" onClick={() => setSelectedTextbook(null)} aria-label="閉じる" className="text-xl leading-none text-neutral-400 hover:text-neutral-700">×</button>
            </div>
            <dl className="mt-5 space-y-3 text-sm">
              ...
              {selectedTextbook.description && <div><dt className="text-neutral-500">説明・補足</dt><dd className="mt-1 whitespace-pre-wrap text-neutral-700">{selectedTextbook.description}</dd></div>}
            </dl>
