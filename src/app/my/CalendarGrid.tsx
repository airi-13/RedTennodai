...
function LessonModal({ selected, periods, onClose }: {
  ...
  if (item.status === "makeup") {
    ...
    return <Overlay onClose={onClose}>
      <div className="space-y-1 text-sm font-medium text-[var(--color-ink)]">
        ...
        <p>{formatDateTime(date, period?.start_time ?? null)}</p>
        <p className="pl-8">↓</p>
        <p>{formatDateTime(item.transferToDate ?? date, destinationPeriod?.start_time ?? null)}</p>
      </div>
    </Overlay>;
  }
  ...
  if (item.status === "makeup_added") {
    ...
    return <MakeupDestinationModal
      selected={selected}
      sourceTime={formatDateTime(item.transferFromDate ?? date, sourcePeriod?.start_time ?? null)}
      destinationTime={formatDateTime(date, period?.start_time ?? null)}
      onClose={onClose}
    />;
  }
  ...
  return <Overlay onClose={onClose}>
    ...
    <NewRegistrationForm date={date} periodId={period?.id ?? ""} periods={periods} />
  </Overlay>;
}

function MakeupDestinationModal({ selected, sourceTime, destinationTime, onClose }: {
  selected: SelectedLesson;
  sourceTime: string;
  ...
  return <Overlay onClose={onClose}>
    <div className="space-y-2">
      <p className="font-bold">振替授業</p>
      <p className="text-sm">{sourceTime}</p>
      <p className="pl-8 text-sm">↓</p>
      <p className="text-sm">{destinationTime}</p>
      <p className="inline-block rounded-full border border-[var(--color-makeup)] px-3 py-1 text-xs font-medium" style={{ color: "var(--color-makeup)" }}>[振替]</p>
    </div>
    <form action={formAction} className="space-y-3">
      <input type="hidden" name="requestType" value="absence" />
      <input type="hidden" name="targetDate" value={selected.date} />
      <input type="hidden" name="targetPeriodId" value={selected.item.periodLabel ? "" : ""} />
      ...
      <button type="submit" disabled={isPending} className="w-full rounded-md py-2 text-sm font-medium text-white disabled:opacity-50" style={{ background: "var(--color-absent)" }}>欠席</button>
...
function ModalHeader({ date, item, onClose }: { date: string; item: Extract<CalendarDayItem, { type: "lesson" }>; onClose: () => void }) {
  return <div className="flex items-start justify-between"><div><p className="font-display font-bold">{date}</p><p className="text-sm text-[var(--color-ink-soft)]">{item.periodLabel} {item.subject}</p></div><button onClick={onClose} className="text-[var(--color-ink-soft)]">✕</button></div>;
}
...
function NewRegistrationForm({ date, periodId, periods }: { date: string; periodId: number | ""; periods: { id: number; name: string; start_time: string | null }[] }) {
  ...
  return <form action={formAction} className="space-y-3">
    ...
    <div className="flex gap-4 text-sm">
      <label className="flex items-center gap-1"><input type="radio" name="requestType" value="absence" checked={requestType === "absence"} onChange={() => setRequestType("absence")} />欠席</label>
      <label className="flex items-center gap-1"><input type="radio" name="requestType" value="makeup" checked={requestType === "makeup"} onChange={() => setRequestType("makeup")} />振替</label>
    </div>
    {requestType === "makeup" && <>
      <div className="space-y-2">
        <label className="block text-xs text-[var(--color-ink-soft)]">振替授業</label>
        ...
        <select name="makeupPeriodId" value={makeupPeriodId} onChange={(e) => setMakeupPeriodId(e.target.value ? Number(e.target.value) : "")} required className="w-full rounded-md border border-[var(--color-border)] px-2 py-1 text-sm">
